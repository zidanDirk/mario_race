...
const dir=await mkdtemp(join(tmpdir(),'loadouts-production-'));
...
try{
 ...
 await page.screenshot({path:'artifacts/loadouts-v1/production-practice.png'});await page.click('#home');await page.click('[data-loadout=drift]');await page.keyboard.press('Escape');await page.click('[data-race-mode=time-trial]');await start();assert.equal(starts[1].rulesVersion,4);assert.equal(starts[1].loadout,'drift');
 ...
 // Actual driving/finish recording is verified separately by loadouts-race-browser.mjs.
 ...
 await page.click('[data-race-mode=grand-prix]');await page.getByRole('button',{name:'用户登录与云端排行榜'}).click();await page.locator('.cloud-board').getByText('极速型',{exact:false}).waitFor();await page.screenshot({path:'artifacts/loadouts-v1/cloud-loadout.png'});
 assert.deepEqual(errors,[]);const report={passed:true,checks:['production build has no QA hooks','signed-in GP binds speed/rules5; TT binds drift/rules4','20 seconds of production practice writes no local or cloud results','HTTP finish fixture preserves bound loadout and rendered leaderboard identifies it'],starts,errors};await writeFile('artifacts/loadouts-v1/production.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));
