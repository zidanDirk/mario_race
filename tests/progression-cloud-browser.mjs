...
async function login(){await page.getByRole('button',{name:'用户登录与云端排行榜'}).click();await page.waitForFunction(()=>!document.querySelector('.cloud-refresh').disabled);await page.getByLabel('本地开发测试（非微信 / Google 登录）').fill('成长联调车手');await page.getByRole('button',{name:'测试登录',exact:true}).click();await page.getByRole('heading',{name:'成长联调车手',exact:true}).waitFor();await page.waitForFunction(()=>document.querySelector('.growth-account').textContent.includes('账号成长'));await page.getByRole('button',{name:'关闭排行榜'}).click();}
