...
export function mountGarage(roster: Driver[], onSelect: (key: string) => void) {
  ...
  dialog.innerHTML = `
    ...
    <p class="guide-gamepad-note"><b>技巧竞速：</b>同向紧跟前车积累尾流，完成后获得短加速。蘑菇赛道后段右侧有两个低跳台，左侧保留平路；起跳前后短窗口内点按漂移键，落地触发技巧加速。一直按住不会重复奖励。<br/><b>道具攻防：</b>对手也会吃箱和使用六类道具。短按 E / Q、道具栏或手柄左肩键投出，长按后挂香蕉或龟壳，松开释放；按住 C 可向后投壳。后挂物抵挡一次攻击后消耗，第二格不会自动释放。来袭时注意方向提示；休闲难度有更长攻击间隔与受击保护，不计标准榜。三重蘑菇占一格、分三次使用；超级喇叭按下立即清除近处道具并击退近车，墙体可阻挡。计时模式没有道具攻防。</p>
