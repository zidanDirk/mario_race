export type Item = 'mushroom' | 'green-shell' | 'red-shell' | 'banana' | 'triple-mushroom' | 'super-horn';
...
const ORDER: Item[] = ['mushroom', 'green-shell', 'banana', 'triple-mushroom', 'super-horn', 'red-shell'];
...
export class ItemInventory {
  ...
  acquire(rank: number, random: () => number): boolean {
    if (!this.canCollect) return false;
    // A small game's catch-up mix, not the original game's probability table.
    const position = Number.isFinite(rank) ? Math.max(1, Math.min(6, rank)) : 1;
    const weights = position <= 2 ? [.12, .28, .32, .03, .13, .12]
      : position <= 4 ? [.24, .20, .14, .16, .10, .16] : [.28, .10, .06, .29, .07, .20];
    let roll = Math.max(0, Math.min(.999999, random()));
