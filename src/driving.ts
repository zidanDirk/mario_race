...
export type Track = { trackLength: number; sample: (t: number, lane?: number) => TrackSample };
export const wrapAngle = (a: number) => Math.atan2(Math.sin(a), Math.cos(a));
export const DRIFT_THRESHOLDS = [.95, 2.05, 3.35] as const;
export const DRIVE_TUNING = { topSpeed: 47, reverseSpeed: 10, offRoadSpeed: 19, boostSpeed: 65, maxRoadLane: 8.8, recoveryDistance: 24 };
...
export class KartDriving {
  ...
  get lap() { return Math.min(3, 1 + Math.max(0, Math.floor((this.nextGate - 1) / 8))); }
  get finished() { return this.nextGate > 24; }
  ...
  reset(t = -.028, lane = -3, speed = 0, gate?: number) {
    ...
    this.hopHeight = this.hopVelocity = 0; this.driftDirection = this.charge = this.boost = this.stun = 0;
  ...
  project() {
    ...
    this.lane = (this.position.x - px) * tz - (this.position.z - pz) * tx;
    this.roadDistance = Math.sqrt(best); this.offRoad = this.roadDistance > DRIVE_TUNING.maxRoadLane;
    this.wrongWay = Math.abs(this.speed) > 5 && this.velocity.x * tx + this.velocity.z * tz < -3;
    // Ordered gates every eighth of a lap prevent off-road shortcuts and reverse-lap exploits.
    const gateT = this.nextGate / 8;
    if (previous <= gateT && this.routeT >= gateT && this.routeT - previous < .02 && this.roadDistance < 10.5 && !this.wrongWay) this.nextGate++;
  ...
  step(input: DriveInput, coins: number, dt: number) {
    this.releasedTurbo = 0; this.recovered = false;
    ...
    if (drifting && !this.offRoad) this.charge = Math.min(3.8, this.charge + dt * (.6 + .6 * Math.max(0, input.steer * this.driftDirection)));
    else if (this.offRoad) this.charge = Math.max(0, this.charge - dt * 1.7);

    const top = this.stun > 0 ? 0 : this.boost > 0 ? DRIVE_TUNING.boostSpeed : this.offRoad ? DRIVE_TUNING.offRoadSpeed : DRIVE_TUNING.topSpeed + coins * .4;
    ...
    const rate = input.brake ? 3.0 : this.offRoad && this.boost <= 0 ? 2.6 : input.throttle ? 1.65 : .9;
    ...
    const slip = drifting ? this.driftDirection * (.25 + .1 * Math.max(0, this.steer * this.driftDirection)) : 0;
    const desiredTravel = this.heading + slip;
    this.travelHeading = wrapAngle(this.travelHeading + wrapAngle(desiredTravel - this.travelHeading) * (1 - Math.exp(-(drifting ? 7 : 14) * dt)));
    this.velocity.set(Math.sin(this.travelHeading) * this.speed, 0, Math.cos(this.travelHeading) * this.speed);
    this.position.addScaledVector(this.velocity, dt); this.position.y = .14;
    this.project();
    const missedGate = this.routeT > this.nextGate / 8 + .055;
    if (this.roadDistance > DRIVE_TUNING.recoveryDistance || missedGate) this.recoveryTimer += dt;
    else this.recoveryTimer = 0;
    if (this.recoveryTimer > 1.2) this.recover();
  }
}
