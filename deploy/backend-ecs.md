...
- `API_ORIGIN`：API 的完整 HTTPS Origin，决定 Google 回调地址。
...
- OAuth 与 SMTP Secret 只保存在服务器的 `.env.production`，不进入镜像和前端构建。
...
# 编辑 APP_ORIGIN、APP_RETURN_PATH、OAuth 与 SMTP 凭据；构建静态站点时保留 VITE_API_ORIGIN
sudo docker compose --env-file .env.production up -d --build
sudo docker compose --env-file .env.production ps
curl --fail https://backend.zhangzidan.com/api/health
```

没有登录服务凭据也能先启动并验证健康检查、空排行榜和数据库；对应登录入口会保持未配置状态。
