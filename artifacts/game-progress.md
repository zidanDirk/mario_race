...
用户反馈「遗漏检查点 · 请沿赛道返回」无法操作。已复现缺少恢复入口，并在规则测试中确认旧安全点可能距漏点约94/114米。新增明确说明、地图橙圈、返回按钮与标准手柄Select；共享Backspace恢复，约8米前正向放车，原门序保留，真实通过才计点，计时继续最多扣3金币。无自动传送，防重复扣费，暂停/退出隔离；手机横竖屏不遮计时/驾驶按钮。构建与完整规则通过，6组真实浏览器输入通过，详细报告 artifacts/checkpoint-assist/README.md。截图不足以还原原行驶轨迹，正常主路/捷径测试未发现误判。


## 机关战术（第四轮方案二）— 完成

城堡湿滑内线/干燥外线与周期双传送带（停车/倒车同样受力、中央普通路面）、炸弹前抛后放/引信范围/护栏遮挡、星星五秒提速免伤与碰车攻击完成。AI 共享路面规则并选路避弹，强道具按名次和领先距离加权且有限频；暂停、重开、退出、杯赛切站和淘汰清理已接入。新增城堡 20 秒试驾，正式成绩与旧规则隔离至 GP6 / TT5。证据见 artifacts/tactics-v1/README.md。

构建、全量规则、28项服务端、11组生产构建交互检查通过。城堡真实键盘三圈63.417秒，0救援/0撞栏，AI真实使用炸弹与星星；手机横竖屏与模拟手柄新道具输入通过。独立预览5180/生产预览5181避开其他项目占用的5173。
